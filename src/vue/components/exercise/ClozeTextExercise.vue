<template>
  <div class="sf-cloze" data-cy="cloze-text-exercise">
    <p class="sf-cloze-text">
      <template v-for="(part, j) in textParts" :key="j">
        <span v-html="part"></span>
        <span
          v-if="j < choices.length"
          class="sf-cloze-dd"
          :class="
            answered ? `sf-cloze-dd--${corr[j] ? 'correct' : 'incorrect'}` : ''
          "
          :data-cy="`dropdown-${j}`"
          @click="toggleOpen(j)"
        >
          ...
          <span
            v-if="answered && !corr[j]"
            class="sf-cloze-dd-reveal"
            >&rarr; {{ ans[j] }}</span
          >
...
// Sentence split on the (N) blank markers — same convention as Gap.
...
function init(): void {
  selections.value = choices.value.map(() => null);
}
...
// length already available from props (choices).
init();
...
// Finalize once every blank has a selection.
watch(
  selections,
  () => {
    if (
